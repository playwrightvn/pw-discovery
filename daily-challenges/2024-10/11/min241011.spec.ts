...
- Hàm `calculateCompoundInterest` cần nhận vào các tham số:
  ...
  - `days`: Số ngày đầu tư.
- Giả sử lãi được cộng gộp hàng ngày (`n = n`).
...
function calculateCompoundInterest(principal, rate, days, n){
    const t = days / n
    const r = rate / 100
    const money = (principal * Math.pow((1 + r / n), n * t)).toFixed(2)
    console.log('Số tiền sau '+days+' ngày là: '+money)
}

calculateCompoundInterest(1000, 5, 30, 365)
calculateCompoundInterest(2000, 3.5, 60, 365)